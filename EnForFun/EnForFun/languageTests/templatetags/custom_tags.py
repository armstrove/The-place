...
def drag_n_drop_html(answer, id_num, rand=False, answered_true=False):
    ...
    if answered_true:
        ...
        for index, splited_strings in enumerate(words_of_answer):
            ...
            drop_answer_string += '<div class="drag-n-drop-aswer-cell" id="{}{}" draggable="false" ondragstart="drag_answer(event)"  ondrop="drop(event)" ondragover="allowDrop(event)">'.format(
                id_num, index) + splited_strings + '</div>'
    else:
        print("Answered true")
        for index, splited_strings in enumerate(words_of_answer):
            if splited_strings in symbols:
                ...
                words_for_drag_n_drop += '<div class="dragNDropWord" id="{}{}" draggable="true" ondragstart="drag(event)" onclick="add_answer(event)">'.format(
                    id_num, index) + splited_strings + '</div>'
            else:
                print("Error: unknown drapNdrop string" + splited_strings)
...
def generate_drag_n_drop_answer_choice_html(answer, answered_val, id_num, answered_true=False):
    ...
    if re.match(r"^\s*$", str(answered_val)):
        print("None or empty")
        [words_for_drag_n_drop, drop_answer_string] = drag_n_drop_html(answer, id_num, rand=True,
...
def excersise_choose_the_word(excersise_text, form, answers):
    ...
    if form.is_bound:
        if error_in_answers:
            status_html = '<div class="wrapper-status"><div class="status explanation-message">{}</div></div>'.format(
                error_html_text)
        else:
...
def excersise_type_the_word(excersise_text, form, answers):
    ...
    if form.is_bound:
        if error_in_answers:
            status_html = '<div class="wrapper-status"><div class="status explanation-message">{}</div></div>'.format(
                format_error_text(error_html_text))
        else:
            status_html = '<div class="wrapper-status"><div class="status success-message">Excelent</div></div>'
    else:
        status_html = ""
    ...
    for sentence in sentences_iterator:
        ...
        if form.is_bound:
            ...
            if form.fields[sentence].get_bound_field(form, sentence).errors:
                ...
                error_html_text = error_html_text + str(counter) + " " + text + "<br>"
                error_in_answers = True
                [words_for_drag_n_drop_gen, drop_answer_string_gen] = generate_drag_n_drop_answer_choice_html(
                    answer=excersise_text, answered_val=value, id_num=id_num)
            else:
                [words_for_drag_n_drop_gen, drop_answer_string_gen] = generate_drag_n_drop_answer_choice_html(
                    answer=excersise_text, answered_val=value, id_num=id_num, answered_true=True)
                pass
