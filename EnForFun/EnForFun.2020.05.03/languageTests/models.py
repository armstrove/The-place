...
class TopicManager(models.Manager):
    def get_by_pk(self,pk):
        qs = self.get_queryset().filter(pk=pk) # languagetest.objetcs = self.get_queryset()
        return qs
...
class Topic(models.Model):
    ...
    def get_absolute_url(self):
        """Returns the url to access a detail record for this Topic."""
        #return reverse('languagetest_detail', args=[str(self.id)]) 
        #return reverse('topic', args=[str(self.id)]) 
        return reverse('topic', args=[self.id]) 
    
class LanguageTestManager(models.Manager):
...
class LanguageTest(models.Model):
    ...
    def Meta(self):
        unique_togather = ("number","topic")
    
    def cal_num(self,topic):
        if self.number:
            return self.number
        
        present_keys = LanguageTest.objects.filter(topic=topic).order_by('-number').values_list('number',flat=True)

        if present_keys:
            vals=LanguageTest.objects.filter(topic=topic).order_by('number').values('number')
            return self.get_number(vals)
        else:
            return 1
        
    def get_number(self,values_list):
        if values_list[0]['number'] != 1:
            return 1
        num=1
        for el in values_list[1:]:
            if el['number'] != num+1:
                return num+1
            else:
                num+=1
    ...
    def __str__(self):
        """String for representing the Model object."""
        return "{}-{}".format(self.number,self.topic)
